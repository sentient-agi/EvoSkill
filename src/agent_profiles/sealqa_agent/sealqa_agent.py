...
def make_sealqa_agent_options(model: str | None = None, provider: str | None = None, prompt_file: Path | None = None):
    """Create a factory function for agent options."""

    def factory() -> Union[ClaudeAgentOptions, dict]:
        return get_sealqa_agent_options(model=model, provider=provider, prompt_file=prompt_file)
